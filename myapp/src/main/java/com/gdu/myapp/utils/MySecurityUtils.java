...
public class MySecurityUtils {
   ...
   * 3. java.security 패키지를 활용한다. 1byte = 4빝트
   */
  public static String getSha256(String original) {
    StringBuilder builder = new StringBuilder();
    
    try {
      MessageDigest digest = MessageDigest.getInstance("SHA-256");    // 존재하지않는 알고리즘이면 exception 이 발생될 것이다.
      digest.update(original.getBytes());
      byte[] bytes = digest.digest();
      for(int i = 0; i < bytes.length; i++) {
        builder.append(String.format("%02X", bytes[i]));    // 1byte 짜리를 16진수로 바꾼다. 한 바이트당 2개의 글자로(%02X) 
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(getRandomString(6, true, false));
