...
<body>
  ...
  <div>
    <form action="${contextPath}/upload1.do"
      ...
      <div>
        <input type="file" name="files" class="files"  accept="image/*" multiple>   <!-- multipartRequest.getFiles -->
      </div>
      <div>
        <input type="text" name="writer" placeholder="작성자">   <!-- multipartRequest.getParameter("") -->
      </div>
      <div>
  ...
  <script type="text/javascript">
    ...
    const fnAfterInsertCheck = ()=>{
    	const insertCount = '${insertCount}';    // 값이 전달되지 않아도 오류가 나지 않도록 무조건 문자열로 받기
    	if(insertCount !== '') {
    		if(insertCount === '1') {
    			alert('저장되었습니다.');
    		} else {
